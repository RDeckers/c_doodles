#include <stdlib.h>
//COMPLETELY TYPE AGNOSTIC LISTS.
//If this macro is called it will define a type listSUFFIX_t which is a list with elements of type TYPE.
//The functions follow the format functionNameListSUFFIX(listSUFFIX_t *, ...).
// e.g. listAppend_woop(list_woop_t *, ...); for DEFINE(_woop, float*)
#define DEFINE_LIST(SUFFIX, TYPE )/*SUFFIX can also be left empty, this will create list_t and listAppend like things*/\
 ...
 int listAppend##SUFFIX(list##SUFFIX##_t *list, TYPE element){\
   list->tail->next = malloc(sizeof(listNode##SUFFIX##_t));\
 ...
 list##SUFFIX##_t* listCreate##SUFFIX(TYPE firstElement){\
   list##SUFFIX##_t* listp = malloc(sizeof(list##SUFFIX##_t));\
   if(listp == NULL) return NULL;\
   listp->head = malloc(sizeof(listNode##SUFFIX##_t));\
